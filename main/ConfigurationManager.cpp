...
bool ConfigurationManager::seedDefaultConfigSections()
{
      ...
      { JSON_LCC_CAN_NODE,
        {
          ...
          { JSON_LCC_CAN_RX_NODE, NOT_A_PIN },
          { JSON_LCC_CAN_TX_NODE, NOT_A_PIN },
#endif
        }
      },
  ...
  if (!csConfig.contains(JSON_HC12_NODE))
  {
    ...
    {
#if CONFIG_HC12
      { JSON_HC12_ENABLED_NODE, (bool)CONFIG_HC12 },
#if HC12_UART_UART1
      { JSON_HC12_UART_NODE, 1 },
#elif HC12_UART_UART2
      { JSON_HC12_UART_NODE, 2 },
#else
#error unsupported HC12 UART selection.
#endif
      { JSON_HC12_RX_NODE, CONFIG_HC12_RTX_PIN },
      { JSON_HC12_TX_NODE, CONFIG_HC12_TX_PIN },
#else
      { JSON_HC12_ENABLED_NODE, false },
      { JSON_HC12_UART_NODE, 1 },
      { JSON_HC12_RX_NODE, NOT_A_PIN },
      { JSON_HC12_TX_NODE, NOT_A_PIN },
#endif
    };
  ...
  if (!csConfig.contains(JSON_HBRIDGES_NODE))
  {
    csConfig[JSON_HBRIDGES_NODE] =
    {
      { CONFIG_OPS_TRACK_NAME,
        {
          { JSON_HBRIDGE_PREAMBLE_BITS_NODE, CONFIG_OPS_PREAMBLE_BITS },
          { JSON_HBRIDGE_RMT_CHANNEL_NODE, RMT_CHANNEL_0 },
          { JSON_HBRIDGE_ENABLE_PIN_NODE, CONFIG_OPS_ENABLE_PIN },
          { JSON_HBRIDGE_SIGNAL_PIN_NODE, CONFIG_OPS_SIGNAL_PIN },
#if defined(CONFIG_OPS_THERMAL_PIN)
          { JSON_HBRIDGE_THERMAL_PIN_NODE, CONFIG_OPS_THERMAL_PIN },
#else
          { JSON_HBRIDGE_THERMAL_PIN_NODE, NOT_A_PIN },
#endif
          { JSON_HBRIDGE_SENSE_PIN_NODE, CONFIG_OPS_ADC },
#if OPS_RAILCOM_ENABLED
          { JSON_RAILCOM_NODE, (bool)OPS_RAILCOM_ENABLED }
#else
          { JSON_RAILCOM_NODE, false }
#endif
        },
      },
      { CONFIG_PROG_TRACK_NAME,
        {
          { JSON_HBRIDGE_PREAMBLE_BITS_NODE, CONFIG_PROG_PREAMBLE_BITS },
          { JSON_HBRIDGE_RMT_CHANNEL_NODE, RMT_CHANNEL_1 },
          { JSON_HBRIDGE_ENABLE_PIN_NODE, CONFIG_PROG_ENABLE_PIN },
          { JSON_HBRIDGE_SIGNAL_PIN_NODE, CONFIG_PROG_SIGNAL_PIN },
          { JSON_HBRIDGE_SENSE_PIN_NODE, CONFIG_PROG_ADC },
        },
      }
    };
    // flag to indicate we need to persist the configuration update
  ...
  if (!csConfig.contains(JSON_RAILCOM_NODE))
  {
    csConfig[JSON_RAILCOM_NODE] =
    {
      ...
      { JSON_RAILCOM_ENABLE_PIN_NODE, CONFIG_OPS_RAILCOM_ENABLE_PIN },
      { JSON_RAILCOM_BRAKE_PIN_NODE, CONFIG_OPS_RAILCOM_BRAKE_ENABLE_PIN },
      { JSON_RAILCOM_SHORT_PIN_NODE, CONFIG_OPS_RAILCOM_SHORT_PIN },
      { JSON_RAILCOM_UART_NODE, CONFIG_OPS_RAILCOM_UART },
      { JSON_RAILCOM_RX_NODE, CONFIG_OPS_RAILCOM_UART_RX_PIN },
#else
      { JSON_RAILCOM_ENABLE_PIN_NODE, NOT_A_PIN },
      { JSON_RAILCOM_BRAKE_PIN_NODE, NOT_A_PIN },
      { JSON_RAILCOM_SHORT_PIN_NODE, NOT_A_PIN },
      { JSON_RAILCOM_UART_NODE, 2 },
      { JSON_RAILCOM_RX_NODE, NOT_A_PIN },
#endif
    };
    // flag to indicate we need to persist the configuration update
    config_updated = true;
...
void ConfigurationManager::configureEnabledModules()
{
#ifdef DEDICATED_MODULE_EXECUTOR
  Service *service = &moduleService;
#else
  Service *service = lccStack->service();
#endif
  parseWiFiConfig();
  ...
  turnoutManager.reset(new TurnoutManager(lccStack->node(), service));
  ...
  LOG(INFO, "[Config] Registering StatusDisplay module");
  statusDisplay_.emplace(lccStack.get(), service);
  LOG(INFO, "[Config] Registering StatusLED module");
